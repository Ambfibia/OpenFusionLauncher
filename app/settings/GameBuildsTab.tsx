...
export default function GameBuildsTab({ active }: { active: boolean }) {
  ...
  const clearGameCache = async (uuid: string, name?: string) => {
    ...
    try {
      ...
      if (ctx.alertError) {
        const msg = name
          ? t("Failed to clear game cache for {name}").replace("{name}", name)
          : t("Failed to clear game cache");
        ctx.alertError(msg + ": " + e);
      }
    }
  };
  ...
  const downloadOfflineCache = async (uuid: string) => {
    try {
      ...
      if (ctx.alertError) {
        ctx.alertError(
          t("Failed to kickoff offline cache download") + ": " + e,
        );
      }
    }
  };
  ...
  const repairOfflineCache = async (uuid: string) => {
    try {
      ...
      if (ctx.alertError) {
        ctx.alertError(
          t("Failed to kickoff offline cache repair") + ": " + e,
        );
      }
    }
  };
  ...
  const deleteOfflineCache = async (uuid: string, name?: string) => {
    ...
    try {
      ...
      if (ctx.alertError) {
        const msg = name
          ? t("Failed to delete offline cache for {name}").replace(
              "{name}",
              name,
            )
          : t("Failed to delete offline cache");
        ctx.alertError(msg + ": " + e);
      }
    }
  };
  ...
  const importBuild = async (manifestPath: string) => {
    try {
      ...
      if (ctx.alertSuccess) {
        ctx.alertSuccess(
          t("Imported build {name}").replace("{name}", newVersionLabel),
        );
      }
      return true;
    } catch (e: unknown) {
      if (ctx.alertError) {
        ctx.alertError(t("Failed to import build") + ": " + e);
      }
    }
    return false;
  };
  ...
  const removeBuild = async (uuid: string, deleteCaches: boolean) => {
    try {
      ...
      if (ctx.alertSuccess) {
        ctx.alertSuccess(t("Removed build {name}").replace("{name}", name));
      }
    } catch (e: unknown) {
      if (ctx.alertError) {
        ctx.alertError(t("Failed to remove build") + ": " + e);
      }
    }
  };
  ...
  const addBuildManual = async (name: string, assetUrl: string) => {
    try {
      ...
      if (ctx.alertError) {
        ctx.alertError(t("Failed to add build") + ": " + e);
      }
    }
  };
